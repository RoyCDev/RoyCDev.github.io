...
function ProjectPage() {
    ...
    return (
        <section className="animate-slide">
            <h1 className="text-2xl font-medium mb-1 flex items-baseline gap-2">
                {project.name}
                <a href={project.link} target="_blank"><FaExternalLinkAlt size={12} /></a>
            </h1>
            <p className="flex items-center gap-2 mb-6">
                <span className="badge badge-soft">{project.year}</span>
                {project?.tools.join(", ")}
            </p>
            <p className="mb-6 text-lg">{project.desc}</p>
